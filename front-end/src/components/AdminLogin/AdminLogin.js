...
async function LogIn(credentials) {
  return await axios
    .post("/logincredentials", credentials)
    .then((res) => res.data);
}
  ...
  return (
    <>
      ...
      <div className={AdminLoginStyle.Box}>
        ...
        <div className={AdminLoginStyle.signin}>
          Sign In
          <br />
          (user: admin, pswd:admin)
        </div>
        <form className={AdminLoginStyle.form} onSubmit={handleSubmit}>
          {elements.orderForm.map((el, ind) => (
            <FormElement
              key={el.elementConfig.placeholder}
              valid={el.valid}
              inputtype={el.inputtype}
              elementConfig={el.elementConfig}
              changes={(event) => changeVal(event, ind)}
            />
          ))}
        </form>
        <span className={AdminLoginStyle.forgot}>
          Forgot email/username or password?<a href="./">Click here!</a>
        </span>
        <div className={AdminLoginStyle.line}></div>
        <span className={AdminLoginStyle.signup}>
          Want to register a new admin account?<a href="./">Call us now!</a>
        </span>
      </div>
    </>
  );
